...
// Prototipo da funcao que carrega uma imagem como uma textura do OpenGL
GLuint LoadTexture(const GLchar* ImageName);
...
glm::vec3 LightPosition(3.0f);

GLfloat InteractiveLightSensitivity = 0.05f;
...
int main()
{
	...
	GLuint SkyboxProgram = ShaderProgramFromFiles("shaders/skyboxVert.glsl", "shaders/skyboxFrag.glsl");

	// Carrega um modelo 3D a partir de um arquivo
	Mesh LoadedMesh = LoadObjToMesh("res/sword.obj", glm::vec3(RGB_INT_DEC(255), RGB_INT_DEC(255), RGB_INT_DEC(255)));
	...
	// Carrega uma textura a partir do arquivo
	GLuint Texture = LoadTexture("res/bricks.png");
	GLuint Normal = LoadTexture("res/normal.png");
...
{	
	...
	for (unsigned int i = 0; i < PositionIndices.size(); i++)
	{
		...
		CurrentVertex.Uv = Uvs[UvIndices[i] - 1];
